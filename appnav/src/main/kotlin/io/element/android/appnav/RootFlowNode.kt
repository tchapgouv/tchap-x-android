...
class RootFlowNode(
    ...
    private var accountExpiredRecoveryJob: Job? = null
    ...
    private fun observeSyncState(sessionId: SessionId, navId: Int) {
        ...
        syncStateJob = lifecycleScope.launch {
            ...
            matrixClient.syncService.syncState
                .onEach { state ->
                    accountExpiredRecoveryJob?.cancel()
                    if (state == SyncState.AccountExpired) {
                        ...
                        if (currentTarget is NavTarget.AccountExpired) {
                            if (accountExpiredRecoveryJob == null || !accountExpiredRecoveryJob!!.isActive) {
                                accountExpiredRecoveryJob = launch {
                                    delay(2500)
